...
public class PracticeRepository(AppDbContext context) : IPracticeRepository
{
    ...
    {
        await context
            .Practices.Where(p => p.Id == id)
            .ExecuteDeleteAsync(cancellationToken: cancellationToken);
    }

    public async Task<PracticeEntity?> GetByIdAsync(
        int id,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task<ICollection<PracticeEntity>> GetListByPageAndLessonAsync(
        int lessonId,
        int page,
        int pageSize,
        CancellationToken cancellationToken = default
    ...
    public async Task<ICollection<PracticeEntity>> GetListByPageAsync(
        int page,
        int pageSize,
        CancellationToken cancellationToken = default
    )
    ...
    public async Task<PracticeEntity?> GetByTitleAsync(
        string title,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task UpdateByIdAsync(
        int id,
        PracticeEntity newPractice,
        CancellationToken cancellationToken = default
    )
    {
        await context
            .Practices.Where(t => t.Id == id)
            .ExecuteUpdateAsync(
                t =>
                    t.SetProperty(t => t.LessonId, newPractice.LessonId)
                        ...
                        .SetProperty(t => t.Level, newPractice.Level),
