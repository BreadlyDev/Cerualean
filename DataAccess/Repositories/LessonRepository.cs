...
internal class LessonRepository(AppDbContext context) : ILessonRepository
{
    ...
    public async Task<LessonEntity?> GetByTitleAsync(
        string title,
        CancellationToken cancellationToken = default
    )
    {
        return await context
            .Lessons.AsNoTracking()
            .FirstOrDefaultAsync(l => l.Title == title, cancellationToken: cancellationToken);
    }

    public async Task<ICollection<LessonEntity>> GetListByPageAndCourseAsync(
        int courseId,
        int page,
        int pageSize,
        CancellationToken cancellationToken = default
    ...
    public async Task<ICollection<LessonEntity>> GetListByPageAsync(
        int page,
        int pageSize,
        CancellationToken cancellationToken = default
    )
    ...
    )
    {
        await context
            ...
            .ExecuteUpdateAsync(
                l =>
                    l.SetProperty(l => l.UpdatedTime, DateTime.UtcNow)
                        ...
                        .SetProperty(l => l.CourseId, newLesson.CourseId),
