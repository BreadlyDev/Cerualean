...
internal class QuestionRepository(AppDbContext context) : IQuestionRepository
{
    ...
    public async Task<QuestionEntity?> GetByIdAsync(
        int id,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task<QuestionEntity?> GetByTitleAsync(
        string title,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task<ICollection<QuestionEntity>> GetListByTestAsync(
        int testId,
        CancellationToken cancellationToken = default
    )
    {
    ...
    )
    {
        await context
            ...
            .ExecuteUpdateAsync(
                q =>
                    q.SetProperty(q => q.TestId, newQuestion.TestId)
                        .SetProperty(q => q.ManyRightOptions, newQuestion.ManyRightOptions)
                        .SetProperty(q => q.Text, newQuestion.Text),
