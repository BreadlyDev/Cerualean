...
public interface ILessonService
{
    ...
    Task<RichLessonDto?> GetWithFullInfoByIdAsync(int id, CancellationToken cancellationToken = default);
    Task<LessonDto?> GetByTitleAsync(string title, CancellationToken cancellationToken = default);
    Task<ICollection<RichLessonDto>> GetListWithFullInfoByPageAndCourseAsync(int courseId, int? page, int? pageSize, CancellationToken cancellationToken = default);
