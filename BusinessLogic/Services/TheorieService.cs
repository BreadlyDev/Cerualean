...
internal class TheorieService(ITheorieRepository theorieRepository) : ITheorieService
{
    public async Task AddAsync(
        CreateTheorieDto theorie,
        CancellationToken cancellationToken = default
    )
    {
        await theorieRepository.AddAsync(
            theorie.ToTheorieFromCreateTheorieDto(),
            cancellationToken
        );
    }
    ...
    public async Task<TheorieDto?> GetByIdAsync(
        int id,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task<TheorieDto?> GetByTitleAsync(
        string title,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task<ICollection<TheorieDto>> GetListByPageAndLessonAsync(
        int lessonId,
        int? page,
        int? pageSize,
        CancellationToken cancellationToken = default
    ...
    {
        var theorieList = await theorieRepository.GetListByPageAndLessonAsync(
            lessonId,
    ...
    public async Task<ICollection<TheorieDto>> GetListByPageAsync(
        int? page,
        int? pageSize,
        CancellationToken cancellationToken = default
    )
    {
        var theorieList = await theorieRepository.GetListByPageAsync(
            page: page ?? PaginationDefaults.DefaultPage,
            pageSize: pageSize ?? PaginationDefaults.DefaultPageSize,
            cancellationToken
        );
    ...
    public async Task UpdateByIdAsync(
        int id,
        UpdateTheorieDto newTheorie,
        CancellationToken cancellationToken = default
    )
    {
        await theorieRepository.UpdateByIdAsync(
            id,
            newTheorie.ToTheorieFromUpdateTheorieDto(),
            cancellationToken
        );
    }
}
