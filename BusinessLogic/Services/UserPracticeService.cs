...
internal class UserPracticeService(IUserPracticeRepository userPracticeRepository)
	: IUserPracticeService
{
	public async Task AddAsync(
		CreateUserPracticeDto userPractice,
		CancellationToken cancellationToken = default
	)
	{
		await userPracticeRepository.AddAsync(
			userPractice.ToPracticeFromCreatePracticeDto(),
			cancellationToken
		);
	}
